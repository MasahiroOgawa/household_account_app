...
const defaultCategories = {
  income: ['salary', 'company_refund', 'country_refund', 'deposit', 'other_income'],
...
const commonPatterns: Record<string, string> = {
  ...
  '出金': 'deposit', 'ATM': 'deposit', '引出': 'deposit', '現金': 'deposit',
  'ＡＴＭ': 'deposit', '引き出し': 'deposit',
...
const detectPatternCategory = (description: string, transactionType: 'income' | 'expense'): string => {
  ...
  if (transactionType === 'income') {
    ...
    if (lowerDesc.includes('atm') || lowerDesc.includes('出金')) return 'deposit';
